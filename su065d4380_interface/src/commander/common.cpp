...
bool CommandUtil::confirmChecksum(const std::string & buf, const int CRC_IDX)
{
  ...
  try {
    expected_crc = std::stoi(buf.substr(CRC_IDX, 2), nullptr, 16);
  } catch (std::invalid_argument &) {
    return false;
  }

  const uint16_t actual_crc = CommandUtil::calcChecksum(buf.data(), CRC_IDX);
...
void CommandUtil::logResponse(const rclcpp::Logger & logger, const RESPONSE_STATE & response)
{
  switch (response) {
    ...
    case RESPONSE_STATE::ERROR_SENDING:
      ...
      break;
    default:
      RCLCPP_ERROR(logger, "Invalid");
      break;
  }
}

void CommandUtil::logError(const rclcpp::Logger & logger, const ERROR_STATE & error)
{
  switch (error) {
    case ERROR_STATE::LOW_VOLTAGE:
      RCLCPP_ERROR(logger, "Low Voltage");
      break;
    case ERROR_STATE::HIGH_VOLTAGE:
      RCLCPP_ERROR(logger, "High Voltage");
      break;
    case ERROR_STATE::INTERNAL_DRIVER_ERROR:
      RCLCPP_ERROR(logger, "Internal Driver Error");
      break;
    case ERROR_STATE::SENSOR_ERROR:
      RCLCPP_ERROR(logger, "Sensor Error");
      break;
    case ERROR_STATE::OVER_CURRENT:
      RCLCPP_ERROR(logger, "Over Current");
      break;
    case ERROR_STATE::INVALID_VELOCITY:
      RCLCPP_ERROR(logger, "Invalid Velocity");
      break;
    case ERROR_STATE::OVER_LOAD:
      RCLCPP_ERROR(logger, "Over Load");
      break;
    case ERROR_STATE::COMMUNICATION_ERROR:
      RCLCPP_ERROR(logger, "Communication Error");
      break;
    case ERROR_STATE::OK:
      RCLCPP_INFO(logger, "No Error");
      break;
