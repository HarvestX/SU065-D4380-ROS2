...
class AGVcontrolNode(Node):
    def __init__(self):
        ...
        self.datapublishtimer = self.create_timer(publishtimer_ms,self.datapublish_callback)
    ...
    def datasend_callback(self):
        ...
        self.tmpsenddata1=self.make_senddata(self.agv.comarray,self.agv.rightwheel.vel_cmd_int,self.agv.leftwheel.vel_cmd_int)
    ...
    def write_param_data(self,paramid,param):
        ...
        senddata = struct.pack('>B2sB4s4s2sB',startid,format(0,'02X').encode(),read_command,format(paramid,'04X').encode(),format(param,'04X').encode(),format(tmpnum,'02X').encode(),end)
